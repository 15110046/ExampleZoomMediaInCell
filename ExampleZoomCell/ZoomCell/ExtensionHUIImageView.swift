...
//        flagIsAddSubview = true
    ...
    func setupUIWhenStartPinch(scale: CGFloat) {
        if scale > config.minZoom, viewContainer == nil {
            ...
            self.image = nil
            NotificationCenter.default.post(name: NSNotification.Name.CellisZooming, object: nil)
        }
    }
    func stopZoom() {
    ...
    func reloadUIWhenPinchChanged(sender: UIPinchGestureRecognizer) {
        ...
        if newScale < config.minZoom {
            ...
            if let maxZoom = config.maxZoom, newScale >= maxZoom  {
                view.transform = CGAffineTransform(scaleX: maxZoom, y: maxZoom)
                sender.scale = 1
            }
            else {
